...
Filters.encoder = function(pixelsImg, pixelsCode) {
  var imageData = Filters.createImageData(pixelsImg.width, pixelsImg.height);
  var codeData = Filters.createImageData(pixelsCode.width, pixelsCode.height);
  var output = Filters.createImageData(pixelsImg.width, pixelsImg.height);
  var dst = output.data;
  var d = pixelsImg.data;
  var e = pixelsCode.data;
  for (var i=0; i<d.length; i+=4) {
    var r = e[i];
    var g = e[i+1];
    var b = e[i+2];
    var gs = 0.3*r + 0.59*g + 0.11*b; // greyScale
    // dst[i] = dst[i+1] = dst[i+2] = gs;
    dst[i] = Math.floor(dst[i] / 4)*4 + gs%4;
    dst[i+1] = Math.floor(dst[i+1] / 8)*8 + Math.floor(gs/4)%8;
    dst[i+2] = Math.floor(dst[i+2] / 8)*8 + Math.floor(gs/32);
    dst[i+3] = d[i+3];
  }
  return output;
};

Filters.decoder = function(pixels, args) {
  var output = Filters.createImageData(pixels.width, pixels.height);
  var dst = output.data;
  var d = pixels.data;
  for (var i=0; i<d.length; i+=4) {
    ...
    var g = d[i+1];
    var b = d[i+2];
    var v = d[i]%4 + Math.floor(d[i+1]/4)%8*4 + Math.floor(d[i+2]/32)*32;
    dst[i] = dst[i+1] = dst[i+2] = v;
    dst[i+3] = d[i+3];
  }
  return output;
...
Filters.laplace = function(pixels) {
  ...
  return Filters.convolve(pixels, [-1,-1,8,-1], true);
};
